...
static void SetViewportAndScissor( void ) {
	...
	/* GX viewport coordinates are top-down. viewportY is GL bottom-up,
	 * so flip it here. Scissor is clamped to the screen. */
	{
		int vx  = backEnd.viewParms.viewportX;
		int vy  = glConfig.vidHeight - ( backEnd.viewParms.viewportY + backEnd.viewParms.viewportHeight );
...
#if defined(WII_NATIVE_GX)
	(void)clearBits;
	/* depth always; colour only when the frontend would (fog/fastsky). No stencil on GX. */
	{
		qboolean clearColor = ( ( backEnd.refdef.maxFarClip > 1 || r_fastsky->integer )
			&& !( backEnd.refdef.rdflags & (RDF_NOWORLDMODEL|RDF_NOSKY) ) );
		GXBE_Clear( clearColor, qtrue, backEnd.refdef.fogColor[ 0 ],
					backEnd.refdef.fogColor[ 1 ], backEnd.refdef.fogColor[ 2 ] );
	}
#else
...
#if defined(WII_NATIVE_GX)
		/* GX has no user clip planes: use an oblique near-plane projection
		 * for this portal view only (the next view reloads the plain one). */
		float	plane[4];
		float	plane2[4];
		float	eyePlane[4];
...
#if defined(WII_NATIVE_GX)
				GXBE_LoadProjectionGL(temp.projectionMatrix);
#else
				qglMatrixMode(GL_PROJECTION);
...
const void	*RB_DrawBuffer( const void *data ) {
	...
	if ( r_clear->integer ) {
#if defined(WII_NATIVE_GX)
		GXBE_Clear( qtrue, qtrue, 1, 0, 0.5 );
#else
