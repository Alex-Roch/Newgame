...
void RB_TestFlare( flare_t *f ) {
	...
	if ( f->id != -1 && !f->cgvisible ) {
		...
		/* windowX/Y are GL window coords (bottom-up); the EFB is top-down. */
		depth = GXBE_PeekDepth( (int)f->windowX, glConfig.vidHeight - 1 - (int)f->windowY );
#else
...
#if defined(WII_NATIVE_GX)
	{
		/* Build the ortho projection directly (GX has no matrix stack). */
...
#if defined(WII_NATIVE_GX)
	/* restore the view's matrices for anything drawn after the flares */
