...
static void R_MipMap (byte *in, int width, int height) {
	...
	if ( !r_simpleMipMaps->integer && width > 1 && height > 1 ) {
...
static void Upload32( int numTexLevels, const textureLevel_t *pics,
						  ...
						  int gxTexnum, int gxWrapClampMode )
{
	int			samples;
...
#if defined(WII_NATIVE_GX)
		ri.Error(ERR_DROP, "Unsupported Texture format: %8x (compressed textures are not supported on GX)", pics[0].format );
#endif
		// compressed texture
	...
	if ( ( scaled_width == width ) && 
		...
		if (!mipmap)
		{
			...
			GXBE_Upload32( data, scaled_width, scaled_height,
			               (int)internalFormat, gxTexnum, gxWrapClampMode, qfalse );
#else
...
#if defined(WII_NATIVE_GX)
	/* GXBE_Upload32 generates the mip chain itself, in place, with R_MipMap:
	 * the same reduction the stock loop below performs. */
	GXBE_Upload32( scaledBuffer, scaled_width, scaled_height,
	               (int)internalFormat, gxTexnum, gxWrapClampMode, mipmap );
	if (0)	/* the GL mip loop below is skipped: GX built the chain in place */
#else
...
image_t *R_CreateImage2( const char *name, int numTexLevels, const textureLevel_t *pic,
	...
	Upload32( numTexLevels, pic,
								...
								&image->uploadWidth,
								&image->uploadHeight,
								(int)image->texnum, glWrapClampMode );

#if defined(WII_NATIVE_GX)
	/* texnum 0 is a real GX slot, so "unbound" must be -1 or the next
	 * GL_Bind of texnum 0 would be skipped while another texture is loaded. */
	glState.currenttextures[glState.currenttmu] = -1;
	GXBE_BindTexnum( glState.currenttmu, -1 );
#else
