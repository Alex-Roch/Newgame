...
Derived from Mayo1970/ioQuake3-wii (tr_gx.c), used with the author's
...
#include "../renderergl1/tr_local.h"     /* Q3 types, tess global, glState, glConfig, etc. */
...
void GXBE_GL_State(unsigned long stateBits)
{
    ...
    /* Alpha test. Spearmint encodes a compare function plus a reference
     * value in percent (GLS_ATEST_FUNC_BITS / GLS_ATEST_REF_BITS). */
    if (diff & GLS_ATEST_BITS) {
        unsigned long func = stateBits & GLS_ATEST_FUNC_BITS;
        int ref = (int)((stateBits & GLS_ATEST_REF_BITS) >> GLS_ATEST_REF_SHIFT);
        u8 gxref = (u8)((ref * 255 + 50) / 100);
        u8 comp;

        switch (func) {
        case GLS_ATEST_GREATER:      comp = GX_GREATER; break;
        case GLS_ATEST_LESS:         comp = GX_LESS;    break;
        case GLS_ATEST_GREATEREQUAL: comp = GX_GEQUAL;  break;
        case GLS_ATEST_LESSEQUAL:    comp = GX_LEQUAL;  break;
        case GLS_ATEST_EQUAL:        comp = GX_EQUAL;   break;
        case GLS_ATEST_NOTEQUAL:     comp = GX_NEQUAL;  break;
        default:                     comp = GX_ALWAYS;  break;
        }
        ...
        if (comp == GX_ALWAYS) {
            ...
            GX_SetAlphaCompare(comp, gxref, GX_AOP_AND, GX_ALWAYS, 0);
            if (!gxState.alphaTestActive) {
