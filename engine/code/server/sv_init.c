...
#ifdef GEKKO
	// loopback-only build: the local link never drops packets, so the
	// single-player sized backlog is enough (2 MB instead of 8 MB)
	if ( 0 ) {
#else
...
#ifdef GEKKO
	if ( 0 ) {	// loopback-only build, see SV_Startup
#else
	if ( !Com_GameIsSinglePlayer() ) {
#endif
		svs.numSnapshotEntities = sv_maxclients->integer * PACKET_BACKUP * MAX_SNAPSHOT_ENTITIES;
